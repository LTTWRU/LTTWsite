...
function derive(site) {
  ...
  site.privacyAnalytics = site.metrikaId
    ? `<p>\n` +
      ...
      `      </p>\n` +
      (site.metrikaWebvisor
        ? `      <p>\n` +
          `        Включён Вебвизор — он записывает сеанс: как вы двигали\n` +
          `        страницу, куда нажимали и <strong>что набирали в полях\n` +
          `        формы</strong>. Запись уходит в Яндекс вместе с остальной\n` +
          `        статистикой. Не хотите этого — ответьте «Не нужно» на вопрос\n` +
          `        внизу страницы или нажмите «Изменить решение» ниже.\n` +
          `      </p>\n`
        : '') +
      `      <p>\n` +
